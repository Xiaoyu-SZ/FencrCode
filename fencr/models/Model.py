...
from pytorch_lightning.loggers import TensorBoardLogger, CSVLogger, TestTubeLogger, WandbLogger
...
class Model(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # self.model_logger = WandbLogger(project='FENCR')
        self.log_dir = None
    ...
    def read_data(self, dataset_dir: str = None, reader=None, formatters: dict = None):
        ...
        if type(reader) is str:
            reader = eval('{0}.{0}'.format(reader))(
                dataset_dir=dataset_dir, reader_logger=self.model_logger)
        self.reader = reader
    ...
    def get_dataset(self, phase):
        if self.reader is None:
            self.model_logger.error(
                'Model.Reader is None, Please read data first. (model.read_data(...))')
            return
    ...
    def dataset_get_dataloader(self, dataset: 'Dataset') -> torch.utils.data.DataLoader:
        ...
        if dataset.phase == VAL_PHASE or dataset.phase == TEST_PHASE or dataset.phase == PREDICT_PHASE:
            return torch.utils.data.DataLoader(
                ...
                collate_fn=dataset.collate_batch, pin_memory=True)
        self.model_logger.error(
            "ERROR: unknown phase {}".format(dataset.phase))
        return None
    ...
    def configure_optimizers(self):
        ...
        if self.l2_bias == 1:
            optimize_dict = [
                {'params': weight_p + bias_p, 'weight_decay': self.l2}]
        else:
        ...
        if optimizer_name == 'gd':
            ...
            self.model_logger.error(
                "Unknown Optimizer: " + self.optimizer_name)
    ...
    def init_logger(self, save_dir=MODEL_DIR, name=None, version=None):
        ...
        if version is None:
            ...
            version = os.path.join(
                model_name, '_'.join([hash_code, random_seed]))
        ...
        logger_add_file_handler(
            model_logger, os.path.join(self.log_dir, LOG_F))
    ...
    def init_trainer(self, save_dir=MODEL_DIR, name=None, version=None, **kwargs) -> pl.Trainer:
        save_dir, name, version = self.init_logger(
            save_dir=save_dir, name=name, version=version)

        default_para = copy.deepcopy(DEFAULT_TRAINER_ARGS)
        default_para.update(kwargs)
        default_para['callbacks'].append(EarlyStopping(
            mode='max', patience=self.es_patience, verbose=True))
        ...
        default_para['callbacks'].append(LitProgressBar(
            refresh_rate=default_para['progress_bar_refresh_rate']))
    ...
    def fit(self, train_data=None, val_data=None, trainer=None, **kwargs):
        if trainer is None:
            trainer = self.trainer if self.trainer is not None else self.init_trainer(
                **kwargs)
    ...
    def validation_step(self, batch, *args, **kwargs):
        ...
        if self.val_metrics is not None and len(self.val_metrics.metrics) > 0:
            ...
            return None
        return {LOSS: self.loss_func(batch, out_dict)}
    ...
    def load_model(self, checkpoint_path=None, hparams_file=None):
        ...
        if checkpoint_path is not None:
            if os.path.exists(checkpoint_path):
                self.model_logger.info(
                    'load model from {}'.format(checkpoint_path))
                checkpoint = torch.load(checkpoint_path)
            else:
                self.model_logger.warning(
                    'checkpoint_path does not exist: {}'.format(checkpoint_path))
        hparams = {}
        ...
        if hparams_file is not None:
            if os.path.exists(hparams_file):
                self.model_logger.info(
                    'load hparams from {}'.format(hparams_file))
                hparams.update(
                    yaml.load(open(hparams_file, 'r'), Loader=yaml.FullLoader))
            else:
                self.model_logger.warning(
                    'hparams_file does not exist: {}'.format(hparams_file))
    ...
    def on_train_end(self) -> None:
        if self.log_dir is not None:
            checkpoint_path = os.path.join(
                self.log_dir, CKPT_DIR, CKPT_F + '.ckpt')
    ...
    def on_train_epoch_end(self, outputs) -> None:
        if len(self.train_metrics_buf) > 0:
            ...
            self.model_logger.debug(
                os.linesep + prefix + format_log_metrics_list(self.train_metrics_buf))
    ...
    def on_validation_epoch_end(self) -> None:
        if len(self.val_metrics_buf) > 0:
            if 0 < self.trainer.val_check_interval < 1:
                ...
                prefix = '[Validation Epoch {:>6}]'.format(
                    '{:.2f}'.format(epoch))
            else:
                epoch = self.trainer.check_val_every_n_epoch * \
                    (len(self.val_metrics_buf) - 1)
