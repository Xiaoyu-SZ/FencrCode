...
def hard_max(values):
    ...
    hard = torch.nn.functional.one_hot(
        index, num_classes=values.size(-1))  # ? * c
...
def gumbel_softmax_func(logits, training, random_r=0.5, eps=1e-20, tau=1.0, hard_r=1.0, hard=True):
    ...
    r = torch.rand(logits.size()[
                   :-1], device=logits.device).le(random_r).unsqueeze(dim=-1)  # ? * 1
    logits = logits + u * r  # ? * c
    ...
    r = torch.rand(logits.size()[
                   :-1], device=logits.device).le(hard_r).unsqueeze(dim=-1).float()  # ? * 1
