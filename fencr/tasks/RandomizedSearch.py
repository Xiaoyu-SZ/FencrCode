...
class RandomizedSearch(object):
    ...
    def add_task_args(parent_parser):
        ...
        parser.add_argument('--trial_nums', type=int, default=12,
                            help='Total numbers of the randomized trails')
    ...
    @staticmethod
    def read_pycommand(file_path,trail_nums):
        ...
        for line in lines:
            ...
            if line.startswith('['):
                ...
                continue
            line = eval(line)
            grid_list = RandomizedSearch.grid_args_list(search_dict)
            # 从grid_list中随机选择trail_nums项

            grid_list = random.sample(grid_list, trail_nums)
            for grid in grid_list:
                commands.append(RandomizedSearch.cmd_update_args(line, grid))
                grids.append(grid)
        # TODO: see how gridsearch pass parameters
        return commands, grids

    @staticmethod
    def find_cuda(cuda_dict, try_max=-1):
        RandomizedSearch.cuda_lock.acquire()
        py3nvml.nvmlInit()
        ...
        if len(cuda_devices) > 0:
            ...
            RandomizedSearch.log_lock.acquire()
            DEFAULT_LOGGER.info('select cuda = {}({}MB)'.format(select, max_mem))
            RandomizedSearch.log_lock.release()
        py3nvml.nvmlShutdown()
        RandomizedSearch.cuda_lock.release()
        return select
        ...
        while try_cnt < try_max:
            try:
                if type(cuda_dict) is str:
                    cmd = RandomizedSearch.cmd_update_args(cmd, [('cuda', cuda_dict)])
                elif NUM_CUDA > 0:
                    cuda = RandomizedSearch.find_cuda(cuda_dict)
                    if cuda >= 0:
                        cmd = RandomizedSearch.cmd_update_args(cmd, [('cuda', cuda)])
                RandomizedSearch.log_lock.acquire()
    ...
    def find_info_in_output(outputs):
        ...
        for c in RandomizedSearch.columns:
            starts = c + ':'
            for line in lines[::-1]:
                line = line.strip()
    ...
    def df_row_info(outputs, cmd, des):
        info = RandomizedSearch.find_info_in_output(outputs)
        info['cmd'] = cmd
    ...
    def df_check_repeat(self, df):
        ...
        for c in ['train_metrics', 'val_metrics', 'test_metrics']:
            idx = RandomizedSearch.columns.index(c)
            ...
            info[RandomizedSearch.columns[idx + 3]] = format_log_metrics_dict(stds) + os.linesep + \
                                                format_log_metrics_dict(sems)
        df.loc[len(df)] = [info[c] if c in info else '' for c in RandomizedSearch.columns]
        for i in range(3):
            df.loc[len(df)] = [''] * len(RandomizedSearch.columns)
        return df

    def df_to_log_csv(self, df):
        if len(df) > 0:
            df.to_csv(os.path.join(LOG_CSV_DIR, self.out_csv), index=False)
            summary_df = df[(df['model_name'] != '') & (df[RandomizedSearch.columns[-1]] == '')]
            summary_csv = self.out_csv.replace('.csv', '.summary.csv')
            summary_df.to_csv(os.path.join(LOG_CSV_DIR, summary_csv), index=False)
            if len(summary_df) > 0:
                self.grid_logger.info('{}:'.format(summary_csv))
                info_c = ['model_name', 'description', 'test_metrics']
                self.grid_logger.info(summary_df[info_c])
            if df[RandomizedSearch.columns[-1]].tolist()[-1] != '':
                self.grid_logger.info('{}:'.format(self.out_csv))
                info_c = ['model_name', 'description', 'version', 'time', 'test_metrics']
                if self.repeat > 1:
                    self.grid_logger.info(df[-(self.repeat - 1):][info_c])
                else:
                    self.grid_logger.info(df[-10:][info_c])
    ...
    def run(self, *args, **kwargs):
        ...
        cmd_list, des_list = [], []
        if self.in_f != '':
            cmd_list, des_list = RandomizedSearch.read_pycommand(os.path.join(CMD_DIR, self.in_f),self.trial_nums)
        elif self.cmd_list is not None:
            grid_list = [] if self.search_dict is None else RandomizedSearch.grid_args_list(self.search_dict)
            cmd_list = [RandomizedSearch.cmd_update_args(cmd, grid) for cmd in self.cmd_list for grid in grid_list]
            des_list = [grid for cmd in self.cmd_list for grid in grid_list]
        if self.repeat > 1:
            cmd_list = [RandomizedSearch.cmd_update_args(cmd, [('random_seed', seed)])
        ...
        if len(cmd_list) > 0:
            ...
            if self.grid_workers > 1:
                ...
                results = pool.imap(RandomizedSearch.single_run, map_args)
                pool.close()

            for idx, result in enumerate(results):
                if self.grid_workers <= 1:
                    result = RandomizedSearch.single_run((cmd_list[idx], self.try_max, cuda_dict))
                RandomizedSearch.log_lock.acquire()
                self.grid_logger.info(os.linesep + 'complete: {}/{}'.format(idx + 1, len(cmd_list)))
                try:
                    info = RandomizedSearch.df_row_info(result, cmd_list[idx], des_list[idx])
                    out_df.loc[len(out_df)] = [info[c] if c in info else '' for c in RandomizedSearch.columns]
                    out_df = self.df_check_repeat(out_df)
                    self.df_to_log_csv(out_df)
