...
def leave_out_csv(all_data_file, dataset_name, leave_n=1, warm_n=5, max_user=-1, neg_thresh=0):
    ...
    """
    dir_name = os.path.join(DATASET_DIR, dataset_name)
    print('leave_out_csv', dir_name, leave_n, warm_n)
    if not os.path.exists(dir_name):  # 如果数据集文件夹dataset_name不存在，则创建该文件夹，dataset_name是文件夹名字
        os.mkdir(dir_name)
    all_data = pd.read_csv(all_data_file, sep='\t')
...
def random_sample_eval_iids(dataset_name, sample_n=1000, include_neg=False):
    ...
    def eval_iids(in_df, user_his):
        ...
        sample_neg = sample_iids(sample_n=sample_n, uids=[uid for uid, label in uls if label > 0],
                                 item_num=item_num, exclude_iids=user_his)
        sample_neg = [[str(i) for i in li] for li in sample_neg]
        eval_c = [sample_neg.pop(0) if label > 0 else [] for uid, label in uls]
            ...
            for idx, (uid, label) in enumerate(uls):
                if label > 0 and uid in user_neg:
                    for i in range(min(len(eval_c[idx]), len(user_neg[uid]))):
                        eval_c[idx][i] = str(user_neg[uid][i])
        eval_c = [','.join(i) for i in eval_c]
...
def generate_empty_ui_file(dataset_name, user=True, item=True):
    dir_name = os.path.join(DATASET_DIR, dataset_name)
    dfs = [read_df(dir_name, f) for f in [TRAIN_FILE, VAL_FILE, TEST_FILE]]
    df = pd.concat([df for df in dfs if df is not None])
    if user:
        user_df = pd.DataFrame({UID: range(df[UID].max() + 1)})
        write_df(user_df, dir_name, USER_FILE, CSV_TYPE)
    if item:
        item_df = pd.DataFrame({IID: range(df[IID].max() + 1)})
        write_df(item_df, dir_name, ITEM_FILE, CSV_TYPE)
