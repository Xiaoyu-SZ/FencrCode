...
def sample_iids(sample_n, uids, item_num, exclude_iids=None, replace=False, verbose=True, item_p=None):
    # if verbose and GLOBAL_ARGS['pbar']:
    if verbose:
        uids = create_tqdm(uids, desc='sample_iids')
    ...
    if item_p is None:
        ...
        iid_buffer = np.random.choice(item_num, size=sample_n * len(uids), replace=True, p=item_p)
    buffer_idx = 0
    result = []
    for uid in uids:
