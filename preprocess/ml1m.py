...
from fencr.utilities.dataset import copy_ui_features, leave_out_csv, random_sample_eval_iids
...
def format_user_feature(out_file):
    ...
    user_df['u_gender_c'] = user_df['u_gender_c'].apply(
        lambda x: defaultdict(int, {'M': 1, 'F': 2})[x])
...
def format_item_feature(out_file):
    ...
    item_df = pd.read_csv(ITEMS_FILE, sep='::', header=None, engine='python',encoding='ISO-8859-1')
    ...
    item_df['i_year_c'] = item_df['i_year_c'].apply(
        lambda x: int(re.search(r'.*?\(([0-9]+)\)$', x.strip()).group(1)))
    for genre in genres:
        item_df['i_' + genre + '_c'] = item_df['i_genres_c'].apply(
            lambda x: 1 if x.find(genre) == -1 else 2)
    item_df = item_df.drop(columns=['i_genres_c'])
    seps = [0, 1940, 1950, 1960, 1970, 1980, 1985] + \
        list(range(1990, int(item_df['i_year_c'].max() + 2)))
    year_dict = {}
    for i, sep in enumerate(seps[:-1]):
        for j in range(seps[i], seps[i + 1]):
            year_dict[j] = i + 1
    item_df['i_year_c'] = item_df['i_year_c'].apply(
        lambda x: defaultdict(int, year_dict)[x])
...
def format_all_inter(out_file, label01=False):
    ...
    inter_df = pd.read_csv(RATINGS_FILE, sep='::',
...
def main():
    ...
    dataset_name = 'ml1m01-1-1'

    leave_out_csv(all_inter_file, dataset_name, warm_n=1, leave_n=1)

    copy_ui_features(dataset_name=dataset_name,
                     user_file=user_file, item_file=item_file)
