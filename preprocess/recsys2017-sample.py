...
# def get_sample_dataset(sample_rate,pos_df,user_df,item_df):
#     sample_user_df = user_df.sample(frac=sample_rate)
#     sample_item_df = item_df.sample(frac=sample_rate)
#     sample_uid_dict = dict(zip(sample_user_df['user_id'],range(1,len(sample_user_df['user_id'])+1)))
#     sample_item_dict = dict(zip(sample_item_df['item_id'],range(1,len(sample_item_df['item_id'])+1)))
#     sample_pos_df = pos_df[pos_df]


def renumber_ids_and_sample(df, old_column, new_column,sample_rate):
    old_ids = sorted(df[old_column].dropna().astype(int).unique())
    print(len(old_ids))
    sample_old_ids = random.sample(old_ids,int(len(old_ids)*sample_rate))
    sample_id_dict = dict(zip(sample_old_ids, range(1, len(sample_old_ids) + 1)))
    sample_id_df = pd.DataFrame({new_column: sample_old_ids, old_column: sample_old_ids})
    sample_id_df[new_column] = sample_id_df[new_column].apply(lambda x: sample_id_dict[x])
    sample_id_df.index = sample_id_df[new_column]
    sample_id_df.loc[0] = [0, '']
    sample_id_df = sample_id_df.sort_index()
    df[old_column] = df[old_column].apply(lambda x: sample_id_dict[x] if x in sample_id_dict else 0)
    df = df.rename(columns={old_column: new_column})
    return df, sample_id_df, sample_id_dict
...
def format_all_inter(sample_neg=1.0):
    ...
    print(len(pos_df))
    pos_df, uid_df, uid_dict = renumber_ids_and_sample(pos_df, old_column='user_id', new_column=UID,sample_rate=0.3)
    print(len(pos_df))
    pos_df, iid_df, iid_dict = renumber_ids_and_sample(pos_df, old_column='item_id', new_column=IID,sample_rate=0.3)
    print(len(pos_df))
    pos_df = pos_df[(pos_df[UID]!=0) & (pos_df[IID]!=0)]
    print(len(pos_df))
...
def main():
    ...
    dataset_name = 'recsys2017-5-1-sample'
